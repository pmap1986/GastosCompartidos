...
namespace ApiGastosCSena.Data
{
    public class ConexionBD
    {
        ...
        private bool AbrirConexion()
        {
            try
            {
                strCadenaCnx = "workstation id=DBGASTOSC2024.mssql.somee.com;packet size=4096;user id=pmap1986_SQLLogin_1;pwd=i5dca5etsb;data source=DBGASTOSC2024.mssql.somee.com;persist security info=False;initial catalog=DBGASTOSC2024;TrustServerCertificate=True";  
                //strCadenaCnx = "Data Source=DESKTOP-UGSFCI7\\SQLEXPRESS;Initial  Catalog=DB_API_DATOS;Integrated Security=True";
            ...
            {
                blnBDAbierta = false;
                strError = "Error al abrir la conexion -" + ex.Message; return false;
            }
        }
        #endregion
        ...
        public bool Consultar(string SentenciaSQL, bool blnCon_Parametros)
        {
            ...
            {
                strError = "Falla en ejecutar comando -" + ex.Message; return false;
            }
        }
        public bool EjecutarSentencia(string SentenciaSQL, bool blnCon_Parametros)
        {
            if (SentenciaSQL == "")
            {
                strError = "No se ha definido la sentencia a ejecutar "; return false;
            }
            if (blnBDAbierta == false)
            {
                if (AbrirConexion() == false)
            ...
            {
                strError = "Error al ejecutar la instrucción -" + ex.Message; return false;
            }
        }
        public bool ConsultarValorUnico(string SentenciaSQL, bool blnCon_Parametros)
        {
            ...
            try
            {
                strVrUnico = Convert.ToString(objCmdBD.ExecuteScalar()); return true;
            }
            catch (Exception ex)
            {
                strError = "Error al ejecutar instrucción -" + ex.Message; return false;
            }
        }
        ...
        {
            // con este booleano se sabe si hay conexion abierta o no  if (blnBDAbierta == false) 
            {
                if (AbrirConexion() == false)
                {
                    return false;
